...
func TestRowsToJsonMapping(t *testing.T) {
	tt := []struct {
		name        string
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			bytes, err := sql2json.RowsToJsonMapping(tc.mockFunc())
...
func TestRowsToJsonReflection(t *testing.T) {
	tt := []struct {
		name        string
		mockFunc    func() *sql.Rows
		expected    []byte
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			bytes, err := sql2json.RowsToJsonReflection(tc.mockFunc())
			if err != nil && err.Error() != tc.expectedErr.Error() {
				t.Errorf("Expected error %v, but got %v", tc.expectedErr, err)
			}
			if string(bytes) != string(tc.expected) {
...
func BenchmarkRowsToJsonMapping(b *testing.B) {
	db, mock, _ := sqlmock.New()
	defer db.Close()
	rows := sqlmock.NewRows([]string{"Column 1", "Column 2"})
	rows.AddRow("Dummy_1", 1)
	...
	for i := 0; i < b.N; i++ {
		_, _ = sql2json.RowsToJsonMapping(rs)
	}

	b.StopTimer()
}

func BenchmarkRowsToJsonReflection(b *testing.B) {
	db, mock, _ := sqlmock.New()
	defer db.Close()
	rows := sqlmock.NewRows([]string{"Column 1", "Column 2"})
	rows.AddRow("Dummy_1", 1)
	...
	for i := 0; i < b.N; i++ {
		_, _ = sql2json.RowsToJsonReflection(rs)
	}
